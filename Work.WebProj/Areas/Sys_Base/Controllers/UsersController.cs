    ...
    public class UsersController : CtrlTSN<ApplicationUser, q_AspNetUsers>
    {
        ...
        public async Task<string> aj_MasterPasswordUpdate(ManageUserViewModel md)
        {
            ...
            try
            {
                ...
                {
                    int userid = int.Parse(EncryptString.desDecryptBase64(Server.UrlDecode(this.UserId)));//反轉編碼取得一般會員ID
                    var db0 = getDB0();
                    var get_user = db0.會員.Find(userid);
                    if (md.ConfirmPassword != md.NewPassword)
                    {
                        rAjaxResult.result = false;
                        rAjaxResult.message = "新密碼與確認密碼不相符。";
                    }
                    else if (md.OldPassword != get_user.密碼)
                    {
                        rAjaxResult.result = false;
                        rAjaxResult.message = "與原密碼不符。";
                    }
                    else if (md.OldPassword == get_user.密碼 && md.NewPassword == md.ConfirmPassword)
                    {
                        get_user.密碼 = md.NewPassword;
