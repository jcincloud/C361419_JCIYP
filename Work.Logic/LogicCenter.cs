...
namespace ProcCore.Business.LogicConect
{
    ...
    public class ReportCenter : LogicCenter
    {
        ...
        public MemberPrint[] memberLabel(q_會員 q)
        {

            var items = db0.會員.AsEnumerable();
            ...
            if (q.hot != null)
            {
                items = items.Where(x => x.發文註記 == q.hot);
            }
        ...
        public MemberPrint[] otherunitLabel(q_OtherUnit q)
        {
            ...
            if (q.category != null)
            {
                items = items.Where(x => x.分類編號 == q.category);
            }

            if (q.state != null)
            {
                items = items.Where(x => x.顯示狀態Flag == q.state);
            }
